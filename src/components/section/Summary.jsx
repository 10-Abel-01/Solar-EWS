...
const Summary = () => {
  ...
  return (
    <section id="summary" className="max-w-6xl mx-auto px-6 py-24 grid grid-cols-1 md:grid-cols-5 gap-10 items-center">
      <div className="space-y-8 md:col-span-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {features.map((item, i) => (
            <div
              key={i}
              className="bg-white p-6 rounded-3xl border border-gray-100 hover:border-emerald-100 hover:shadow-lg hover:shadow-emerald-50/50 transition-all group"
            >
              <div className="flex justify-between items-start mb-4">
                <div className="text-2xl font-black text-gray-200 font-mono group-hover:text-emerald-100 transition-colors">
                  {item.num}
                </div>
      ...
      <div className="md:col-span-2 bg-gray-950 rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl shadow-gray-200 group h-full flex flex-col justify-center">
        ...
        <h2 className="text-2xl font-bold mb-5 relative z-10 flex items-center gap-3">
          <Zap size={20} className="text-emerald-400" /> Core Functions
        </h2>
        
        <div className="space-y-6 relative z-10">
          <div className="border-l-2 border-emerald-500 pl-4">
            <h5 className="text-emerald-400 font-bold text-sm uppercase tracking-widest">Smart Rotation</h5>
        ...
        <Zap className="absolute -bottom-10 -right-10 text-emerald-500/10 w-48 h-48 group-hover:text-emerald-500/20 group-hover:scale-110 transition-all duration-500" />
      </div>
    </section>
  );
};
